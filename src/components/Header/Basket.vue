<template>
    <div class="basket col-container active" id="basket">
        <form class="basket-popup col-container" ref="basketPopup" name="basket-form" action="">
            <button class="basket-popup-close" @click="basketPopupHandler">X</button>
            <BasketProduct v-for="coffee of getCoffees" :product="coffee" :key="coffee.id" />
            <button class="basket-submit" name="basket-submit" type="submit">Замовити</button>
        </form>
        <button class="basket-icon" @click="basketPopupHandler">
            <img
                class="icon"
                src="data:image/png;base64,"
            />
            <span class="count">1</span>
        </button>
    </div>
</template>

<script>
import { mapGetters } from 'vuex';

import BasketProduct from '@/components/Header/BasketProduct';

export default {
    name: 'Basket',
    components: {
        BasketProduct,
    },
    computed: {
        ...mapGetters(['getCoffees']),
    },
    methods: {
        basketPopupHandler() {
            const basketPopup = this.$refs.basketPopup;
            basketPopup.classList.toggle('visible');
        },
    },
};
</script>

<style lang="scss" scoped>
#basket {
    justify-content: center;
    align-items: center;
}

// Tablet version
@include tabletVersion {
    #basket {
        height: map_get($icon_size, 'medium') * 1.5;
        .basket-popup {
            .basket-submit {
                font-size: map_get($font_size, 'medium');
            }
        }
        .basket-icon {
            .count {
                font-size: map_get($font_size, 'medium');
            }
            .icon {
                height: map_get($icon_size, 'medium') * 1.5;
            }
        }
    }
}

// Desktop version
@include desktopVersion {
    #basket {
        height: map_get($icon_size, 'big') * 1.5;
        .basket-icon {
            .icon {
                height: map_get($icon_size, 'big') * 1.5;
            }
        }
    }
}
</style>
